        ...
        <Card className="mt-6 flex min-h-80 items-center justify-center p-6 text-center">
          <div>
            ...
            <p className="mt-1 max-w-md text-sm leading-6 text-slate-500">
              Run a test call first, then return here to review the transcript and actions.
            </p>
  ...
  return (
    <main className="mx-auto max-w-[1180px] px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mt-5 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-950 sm:text-3xl">Call review</h1>
          <p className="mt-2 flex items-center gap-2 text-sm text-slate-500">
            <Clock3 className="size-4" /> {new Date(call.startedAt).toLocaleString()}
          </p>
        </div>
        ...
        <div className="space-y-5">
          <Card className="p-5">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Detail label="Owner messages" value={String(alertCount)} icon={<Bell className="size-4" />} />
            </div>
          </Card>

          <Card className="overflow-hidden">
            <div className="flex items-center gap-2 border-b border-slate-100 px-5 py-4">
              <Headphones className="size-4 text-slate-600" />
              ...
              {call.transcript.map((turn) => (
                <div key={turn.id} className={cn("flex gap-3", turn.speaker === "caller" && "flex-row-reverse")}>
                  <span className={cn(
                    ...
                    turn.speaker === "caller" ? "bg-[#17152a] text-white" : "bg-slate-100 text-slate-700",
                  )}>
                    {turn.speaker === "caller" ? "C" : "A"}
                  </span>
                  <div className={cn(
                    "max-w-[78%] rounded-lg px-4 py-3 text-sm leading-6",
                    turn.speaker === "caller" ? "rounded-tr-md bg-slate-100 text-slate-700" : "rounded-tl-md border border-slate-200 bg-white text-slate-700",
                  )}>
                    {turn.text}
                  </div>
                </div>
        ...
        <aside className="space-y-5">
          <Card className="p-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-slate-900">Actions created</h2>
            </div>
          ...
          <Link href="/dashboard/answering-setup" className="block rounded-lg border border-slate-200 bg-slate-50 p-5 text-sm font-semibold text-slate-800 hover:bg-white">
            Improve how future callers are answered <ArrowRight className="ml-1 inline size-4" />
          </Link>
        </aside>
      </div>
    </main>
...
function BackLink() {
  return (
    <Link href="/dashboard/calls" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-slate-900">
